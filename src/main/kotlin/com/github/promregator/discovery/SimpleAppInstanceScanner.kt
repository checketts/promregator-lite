...
class SimpleAppInstanceScanner(
    ...
    fun determineInstancesFromTargets(targets: List<AppResponse>,
        ...
        val instances = targets.flatMap { target ->
            val accessURL = determineApplicationRoute(target.spaceAppSummary?.urls, target.originalTarget?.cachedPreferredRouteRegexPattern)
            val domainId = target.spaceAppSummary?.routes?.firstOrNull { rt ->
                accessURL?.startsWith(rt.host + "." + rt.domain.name) ?: false
            }
            val isInternal = domainId?.domain?.internal ?: false

            val numberOfInstances = target.spaceAppSummary?.instances ?: 0
